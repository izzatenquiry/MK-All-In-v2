...
import { getBackendApiRequests, clearBackendApiRequests, type ApiRequestUser } from '../../../services/tokenBackendService';
...
const ApiRequestsView: React.FC<ApiRequestsViewProps> = ({ language }) => {
  ...
  const handleReset = async () => {
    ...
    setResetting(true);
    try {
      const result = await clearBackendApiRequests();
      if (result.success) {
    ...
    <div className="bg-white dark:bg-neutral-900 p-6 rounded-lg shadow-sm h-full overflow-y-auto">
      <div className="mb-6 flex items-center justify-between">
        ...
        <button
          ...
          className="flex items-center gap-2 px-4 py-2 bg-red-600 hover:bg-red-700 dark:bg-red-700 dark:hover:bg-red-600 text-white rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed"

...
const SalesHistoryView: React.FC<SalesHistoryViewProps> = ({
  ...
  const filteredSales = useMemo(() => {
    ...
    if (paymentStatusFilter !== 'all') {
      filtered = filtered.filter(account => account.payment_status === paymentStatusFilter);
    }
  ...
  const calculateTotalRevenue = () => {
    return filteredSales
      .filter(sale => sale.payment_status === 'paid' || !sale.payment_status)
      .reduce((sum, sale) => sum + (Number(sale.sale_price) || 0), 0);
  };
  ...
  const handleExportCSV = () => {
    const headers = ['Email', 'Buyer Name', 'Buyer Email', 'Sale Date', 'Sale Price', 'Payment Method', 'Payment Status'];
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {filteredSales.map((sale) => (
                <tr key={sale.id} className="border-b border-neutral-100 dark:border-neutral-800 hover:bg-neutral-50 dark:hover:bg-neutral-800/50">
                  <td className="p-3 font-medium text-neutral-900 dark:text-white">{sale.email}</td>
                  <td className="p-3">
                    {sale.buyer_name ? (
                      <div>
                        <div className="font-medium text-neutral-900 dark:text-white">{sale.buyer_name}</div>
                        {sale.buyer_email && (
                          <div className="text-xs text-neutral-500 dark:text-neutral-400">{sale.buyer_email}</div>
                        )}
                        {sale.buyer_phone && (
                          <div className="text-xs text-neutral-500 dark:text-neutral-400">{sale.buyer_phone}</div>
                        )}
                      </div>
                    ) : (
                  ...
                  <td className="p-3 text-neutral-600 dark:text-neutral-400">{formatDate(sale.sale_date)}</td>
                  <td className="p-3 font-medium text-neutral-900 dark:text-white">{formatCurrency(sale.sale_price)}</td>
                  <td className="p-3 text-neutral-600 dark:text-neutral-400">{sale.payment_method || '-'}</td>
                  <td className="p-3">{getPaymentStatusBadge(sale.payment_status)}</td>
                </tr>
              ))}
            </tbody>
          </table>

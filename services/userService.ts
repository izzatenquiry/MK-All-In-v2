...
export const getAllUsers = async (): Promise<User[] | null> => {
    ...
    // For ESAIE: email_code is already in users table, so no need to query token_ultra_registrations
    // For MONOKLIX: email_code comes from token_ultra_registrations table
    const isEsaie = BRAND_CONFIG.name === 'ESAIE';
    const emailCodeMap = new Map<string, string | null>();

    if (!isEsaie) {
        // MONOKLIX: Get email_code from token_ultra_registrations
        try {
            const { data: registrations, error: regError } = await supabase
                .from('token_ultra_registrations')
                .select('user_id, email_code')
                .order('registered_at', { ascending: false });

            if (!regError && registrations) {
                registrations.forEach(reg => {
                    if (!emailCodeMap.has(reg.user_id)) {
                        emailCodeMap.set(reg.user_id, reg.email_code);
                    }
                });
            }
        } catch (error) {
            console.log('[getAllUsers] Error accessing token_ultra_registrations');
        }
    }

    return (data as UserProfileData[]).map(profile => {
        const user = mapProfileToUser(profile);
        // For ESAIE: email_code and subscription_expiry are already in user profile from users table (mapped by mapProfileToUser)
        // For MONOKLIX: override email_code with value from token_ultra_registrations (if available)
        // Note: subscription_expiry for both brands comes from users.subscription_expiry (mapped in mapProfileToUser)
        const emailCode = isEsaie ? user.email_code : (emailCodeMap.get(user.id) || user.email_code || null);
        return { ...user, email_code: emailCode };
        // subscriptionExpiry is already included in user object from mapProfileToUser (from users.subscription_expiry)
    });
};
...
export const registerTokenUltra = async (
  ...
  try {
    ...
    // Check if registration already exists (only fetch id and email_code)
    ...
    if (existingRegistration) {
      // Update existing registration - only update fields that can change
      ...
      // New registration: assign email code from pool first
      ...
      // Create new registration - only send registered_at and expires_at (created_at handled by DB)
      ...
      if (insertError) {
        // If insert fails and we assigned an email code, we should rollback (decrement count)
        // But for simplicity, we'll just log it. Admin can fix manually if needed.
...
export const getTokenUltraRegistration = async (
  ...
  try {
    // For ESAIE: token_ultra_registrations table doesn't exist, return null
    const { BRAND_CONFIG } = await import('./brandConfig');
    const isEsaie = BRAND_CONFIG.name === 'ESAIE';
    if (isEsaie) {
      // ESAIE doesn't have token_ultra_registrations table, skip
    ...
    if (data) {
      ...
      // Update status if changed (no updated_at, let DB handle it)
      if (data.status !== status) {
        await supabase
          .from('token_ultra_registrations')
          .update({ status: status as 'active' | 'expired' | 'expiring_soon' })
          .eq('id', data.id);
...
export const hasActiveTokenUltraWithRegistration = async (
  ...
  try {
    // For ESAIE: token_ultra_registrations table doesn't exist, return inactive
    const { BRAND_CONFIG } = await import('./brandConfig');
    const isEsaie = BRAND_CONFIG.name === 'ESAIE';
    if (isEsaie) {
...
export const removeUser = async (userId: string): Promise<{ success: boolean; message?: string }> => {
    try {
        // Step 1: Try to cleanup token_ultra_registrations (gracefully handle if table doesn't exist for ESAIE)
        try {
            ...
            if (registration?.email_code) {
                // Reset email code from user (this will decrement flow account user count)
                await resetEmailCodeFromUser(userId).catch(err => {
                    console.warn("Failed to reset email code during user removal:", err);
                    // Continue anyway
                });
            }

            // Delete all token_ultra_registrations for this user
            const { error: regError } = await supabase
                .from('token_ultra_registrations')
                .delete()
                .eq('user_id', userId);

            if (regError && regError.code !== '42P01') { // 42P01 = table does not exist
                console.warn("Failed to delete token_ultra_registrations:", getErrorMessage(regError));
            }
        } catch (error: any) {
            // Table doesn't exist (e.g., ESAIE) - continue without cleanup
            if (error?.code !== '42P01') {
                console.log('[removeUser] token_ultra_registrations table not available, skipping cleanup');
            }
        }

        // Step 2: Try to cleanup activity_log (gracefully handle if table doesn't exist for ESAIE)
        try {
            ...
            if (activityError && activityError.code !== '42P01') { // 42P01 = table does not exist
                console.warn("Failed to delete activity_log:", getErrorMessage(activityError));
            }
        } catch (error: any) {
            // Table doesn't exist (e.g., ESAIE) - continue without cleanup
            if (error?.code !== '42P01') {
                console.log('[removeUser] activity_log table not available, skipping cleanup');
            }
        }

        // Step 3: Delete the user (main operation - this table must exist)
        const { error } = await supabase
            .from('users')
            .delete()
            .eq('id', userId);

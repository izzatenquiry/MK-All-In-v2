...
- [ ] Supabase connects to ESAIE project (`ttohyacakvrdtuarqbwf.supabase.co`)
...
- [ ] Supabase connects to MONOKLIX project (`xbbhllhgbachkzvpxvam.supabase.co`)
...
if (BRAND_CONFIG.name === 'ESAIE') {
  // ESAIE: Query users table directly
  const { data } = await supabase.from('users').select('*');
} else {
  // MONOKLIX: Query token_ultra_registrations
  const { data } = await supabase.from('token_ultra_registrations').select('*');
}
```
